...
async function startDownload(jobId, url) {
    ...
    const args = [
        ...
        outputTemplate,
        '--no-playlist',
        // Force mp4 for compatibility if desired, but 'best' is usually preferred.
        // If we want to ensure it's a single file we might want -f "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4] / bv*+ba/b"
        // But defaults are usually fine.
        // Let's stick to defaults but maybe restrict to mp4 if they want to play it easily.
        // The user didn't specify format.
    ];
    ...
    try {
        ...
        eventEmitter.on('progress', (progress) => {
            // progress object: { percent, totalSize, currentSpeed, eta }
        ...
        eventEmitter.on('close', async () => {
            ...
            // Since we used a template, we need to find the file with the jobId name
            try {
                ...
                if (file) {
                    ...
                    try {
                        ...
                        console.log(
                            `S3 upload completed for job ${jobId}: ${s3Url}`
                        );
